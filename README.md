...
Our first product is a set of annual **land use and land cover (LULC)** maps and training benchmarks — maps that show what covers the ground (forest, savanna, shaded agroforestry, farmland, water, cities, mines) and how it changes over time. We focus on solving hard Central African Earth observation challenges: persistent cloud cover and separating natural tropical forests from complex multi-layered tree crops.
...
4. **Validation reports** — honest accuracy numbers and area-adjusted matrices for every release
