...
2. **Sensor-fusion core** — combining optical reflectance (Sentinel-2) with microwave structural sensitivity (Sentinel-1 SAR) to resolve cloud-obscured canopies and shaded agroforestry structures.
3. **Portable** — code must not be locked to one vendor. The Google Earth Engine (GEE) code is the reference implementation, but the pipeline design (mosaic → features → classify → filter → validate → publish) can be re-implemented with open tools (STAC + xarray + Dask) if we ever need to leave GEE.
4. **Everything as code** — pipelines, parameters, and documentation live in Git. Data lives in object storage, referenced by manifests.
5. **Simple over clever** — a Random Forest that the whole community understands beats a deep model nobody can maintain.

## 2. Components

| Layer | Tool (free tier) | Role |
|---|---|---|
| Processing | Google Earth Engine (non-commercial) | Satellite mosaics, SAR metrics, features, classification, filters |
...
| Viewer | MapLibre on Cloudflare Pages | Public map browsing |
...
│   ├── features/         # Stage 3: indices, SAR cross-ratio, terrain
...
```
Satellite archives (Sentinel-1/2, Landsat, SRTM — free)
        │
        ▼
[GEE] Cloud-free annual mosaics & SAR dual-pol medians per tile (7 regions)
        │
        ▼
[GEE] Feature stack (bands + indices + SAR VH/VV ratio + terrain)
        │
        ▼
[GEE] Random Forest classification (per region, 10 classes)
        │  trained on community-labelled samples
        ▼
[GEE] Spatial + temporal filters
        │
        ▼
[Python] Validation against independent samples → accuracy report (Olofsson protocol)
        │
        ▼
